			...
			if(file_in_table == NULL){
				...
				if(memcmp(pkt -> file_table[i].md5, file_in_table -> md5,MD5_LEN) != 0){
					...
					for(;i<file_in_table->peer_ip_num;i++){
						if(memcmp(file_in_table -> newpeerip[i],pkt -> file_table[i].newpeerip, IP_LEN) == 0){
							break;
						}
					}
					pthread_mutex_lock(file_table_mutex);
					if(i==file_in_table->peer_ip_num){
						memcpy(file_in_table -> newpeerip[file_in_table->peer_ip_num], pkt -> file_table[i].newpeerip, IP_LEN);
...
void broadcastFileTable(){
	...
	while(peerItr != NULL){
		...
		int ret = send(peerItr -> sockfd, pkt, sizeof(ptp_tracker_t),0);
		if(ret<0){
...
void *handshake_handler(void* arg){
	...
	while(1){
		...
		//printf("sockfd %d\n", current -> sockfd);
		//printf("size %lu\n", sizeof(ptp_peer_t));
		int a = recv(current->sockfd, pkt,sizeof(ptp_peer_t),0);
		if(a < 0){
...
int main(){
	...
	*/
	...
	while(1)
	{
		...
		recv(connfd, p_pkt,sizeof(ptp_peer_t),0);
		...
		if(p_pkt -> type == REGISTER){
			...
			send(connfd, response,sizeof(ptp_tracker_t),0);
			printf("send ptp_tracker_t connfd:%d\tinterval:%d\tpiece_len:%d\tfile_table_size:%d\n",connfd,
					response->interval,response -> piece_len,
