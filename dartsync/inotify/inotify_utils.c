...
void handle_event (queue_entry_t event)
{
  ...
  const char* ext = get_filename_ext(cur_event_filename);
...
*/
    ...
    case IN_DELETE:
      ...
      if(strcmp(cur_event_file_or_dir, "File") == 0){
        Node * fte;
        if(  (fte = findFileEntryByName(cur_event_filename)) != NULL) {
          fte -> status = REMOVED;
          fte->action_time = getCurrentTime();
          sendFileUpdate();
        } else {
          printf ("CANNOT FIND FILE %s IN TABLE\n", cur_event_filename);
        }
    ...
    case IN_CREATE:
      ...
      if(strcmp(cur_event_file_or_dir,"Dir") == 0){
        ...
        Node* newEntry = createNode(cur_event_filename, DONE, getCurrentTime());
        if(newEntry!=NULL){
  ...
  if (wd < 0)
	...
	{  
		...
		{ 
			...
			//	free(newEntry);
	      			Node* newEntry = createNode(curdir, DONE, getCurrentTime());
	      			if(newEntry!=NULL){
	      				appendFileEntry(newEntry);
				}
			}
