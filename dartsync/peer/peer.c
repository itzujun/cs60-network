	...
	while(1){
		...
		for(i=0;i<pt->file_table_size;i++){
		  ...
		  if(strlen(fte->name) == 0) {
			...
			while(p ){
				if(strcmp(p->name,fte->name)==0 ){
				  if(fte->status == REMOVED || p->status == REMOVED){
			      remove(p->name);
			      need_update = 0;
				  }
					break;
			...
			if(need_update && (p == NULL || p->action_time < fte->action_time)){
			  printf(">>>>>>>>>>>>>>>>>>>>> goint to update file %s\n", fte->name);
				Node* newEntry = createNode(p->name, UPDATING, fte->action_time);
  			if(newEntry!=NULL){
					appendFileEntry(newEntry);
				}
				pthread_t ptp_transfer_thread;
				while(pthread_create(&ptp_transfer_thread,NULL,p2p_download_start,(void*)fte) < 0)
				{
				    sleep(WAIT_FOR_THREAD_SRC);
				}
			} else {
			  puts("-----------------------this file don't update---------------------------");
  			Node* itr = fte;
