...
void updateFileTable(ptp_peer_t * pkt){
  ...
  for(k = 0; k < pkt -> file_table_size; k++){
	...
	if(itr == NULL){
		...
		while(i < numOfFiles){
		  if(pkt -> file_table[i].status == REMOVED){
		    i++;
		    continue;
		  }
			//foreach file
			Node * newEntry = malloc(sizeof(Node));
			//memcpy(newEntry, &pkt -> file_table[i], sizeof(Node));
			...
			newEntry->status = pkt -> file_table[i].status;
			newEntry->action_time = pkt -> file_table[i].action_time;
		...
		while(i < numOfFiles){
			...
			}else if(file_in_table -> action_time <= pkt->file_table[i].action_time) {
			  // if exsited, determine whetther it is removed
			  
			  if(pkt->file_table[i].status == REMOVED){
			    file_in_table -> status = REMOVED;
			    file_in_table -> action_time = pkt->file_table[i].action_time;
			    int i=0;
				  ...
				  // if found the ip in ip list
				  if(i != file_in_table->peer_ip_num){
				    int j = i + 1;
				    for(; j < file_in_table->peer_ip_num; j++){
				       memcpy(file_in_table->newpeerip[j-1], file_in_table->newpeerip[j], IP_LEN);
				    }
				    memset(file_in_table->newpeerip[j-1], 0, IP_LEN);
	...
	itr =filetable_head;
	while(itr){
	  if(itr->status == REMOVED && itr->peer_ip_num == 0){
