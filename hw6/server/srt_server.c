  ...
  while(1) {
    if(snp_recvseg(overlay_conn, &srcNodeId, segPtr) == 1){
      ...
      if(segPtr->header.type == SYN){
        ...
        tcb_table[sockfd]->client_nodeID = srcNodeId;
        ...
        pthread_mutex_unlock(tcb_table[sockfd]->bufMutex);
        printf("<func: %s>: send back DATAACK %d:\n", __func__, ackNum);
...
int init_tcb(int sockfd, int port) {
  ...
  tcb->svr_nodeID = topology_getMyNodeID(); 
  tcb->svr_portNum = port;   
  tcb->client_nodeID = 0;  // currently unused until a pkt from client received
