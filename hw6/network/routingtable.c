...
{
  if(routingtable != NULL) {
    routingtable_entry* entryPtr = routingtable_getClosestEntry(routingtable, destNodeID);
    if(entryPtr->destNodeID == NULL) {  // not found and the list is empty
      entryPtr = (routingtable_entry*)malloc(sizeof(routingtable_entry));
      entryPtr->next = NULL;
    } else if(entryPtr->destNodeID != destNodeID) { // not found though the list is not empty
      entryPtr->next = (routingtable_entry*)malloc(sizeof(routingtable_entry));
...
routingtable_entry* routingtable_getClosestEntry(routingtable_t* routingtable, int destNodeID) {
  routingtable_entry* entryPtr = routingtable->hash[makehash(destNodeID)];
  while(entryPtr != NULL 
    && entryPtr->next != NULL 
    && entryPtr->destNodeID != destNodeID) {
    entryPtr = entryPtr->next;
...
void routingtable_print(routingtable_t* routingtable)
{
  if(routingtable != NULL) {
    ...
    printf("%s:\n", __func__);
    for (i = 0; i < MAX_ROUTINGTABLE_SLOTS; i++) {
      routingtable_entry* entryPtr = routingtable[i];
      while(entryPtr != NULL)
