...
void* p2p_download(void* arg) {
  ...
  printf("%s:\t\t DOWNLOAD THREAD FOR FILE %s PIECE %d STARTED", __func__, req->name, req->pNum);

   while(getIPFromFfiletable(peerIP, req->name, fte) < 0){
  ...
  if((reqSock = connectToRemote(peerIP, req->destPort)) < 0) {
...
*/
  peer_peertable_add(peerIP, req->name, req->file_time_stamp, reqSock);
  ...
  if((p2pRecvPkt(reqSock, piece, pieceLen) )< 0) {
    ...
    close(reqSock);
    peer_peertable_rm(peerIP, req->name);
    return NULL;
  }
